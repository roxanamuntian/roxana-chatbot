  ...
  <section class="flex flex-col min-w-[520px] w-full">
    ...
    >
      ...
      <div
        ...
        :class="{ 'flex-row-reverse': !message.isRoxana }"
      >
        <!-- Profile Image -->
        <img
          v-if="message.isRoxana"
...
import { type ThreadMessage } from "openai/sources/beta/threads/messages/messages.mjs";
  ...
  onResponse({ response }) {
    response._data.data.reverse().forEach((element: ThreadMessage) => {
      ...
      if (content?.type == "text") {
        messages.value.push({
          name: element.role == "assistant" ? "Roxana" : customerInitials.value,
          isRoxana: element.role == 'assistant',
